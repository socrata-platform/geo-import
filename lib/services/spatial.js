...
class SpatialService {
  ...
  _upsertLayers(req, res, core, datasets) {
    ...
    return async.map(upsertSpecs, core.upsert.bind(core), (err, openRequests) => {
      return async.map(openRequests, ([[layer, fourfour], upsertRequest], cb) => {
        //layer.read() opens a stream of rows that make up the layer
        layer.read()
        .pipe(es.map((row, cb) => cb(false, JSON.stringify(row))))
        .pipe(upsertRequest)
        .on('response', (upsertResponse) => {
          ...
          core.bufferResponse((bufferedResponse) => {
            ...
            return cb(false, [[layer, fourfour], bufferedResponse]);
          })(upsertResponse);
        });
        ...
        var upsertResult = upsertResponses.map(([[layer, fourfour], response]) => {
          return {
            'uid': fourfour,
            'created': layer.count
          };
        });
        return res.status(202).send(upsertResult);
      });
    });
  }
