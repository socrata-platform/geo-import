...
class SpatialService {
  ...
  _upsertLayers(req, res, core, datasets) {
    ...
    return async.map(upsertSpecs, core.upsert.bind(core), (err, upsert) => {
      ...
      return async.map(upsert, ([[layer, fourfour], upsertRequest], cb) => {
        req.log.info(`Starting upsert to ${fourfour}`);
        layer.pipe(upsertRequest)
          .on('response', (upsertResponse) => {
            req.log.info(`Got upsert response ${upsertResponse}`);
        ...
        err && req.log.info(`Upsert response error: ${err}`);
  ...
  post(req, res) {
    req.log.info("Post to spatial service!")
