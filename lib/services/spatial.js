...
class SpatialService {
  ...
  _createColumns(req, res, core, datasets) {
    ...
    return async.map(colSpecs, core.addColumn.bind(core), (err, colResponses) => {
      if (err) {
        let fourfours = colSpecs.map(([ff, _unused]) => ff);
        return this._destroyLayers(fourfours, core, () => res.status(err.statusCode).send(err.body));
      }
  ...
  _upsertLayers(req, res, core, datasets) {
    ...
    var fail = _.once((code, reason) => {
      var fourfours = datasets.map(([_layer, ds]) => ds.body.id);
      this._destroyLayers(fourfours, core, () => {
          ...
          .on('response', (upsertResponse) => {
            ...
            core.bufferResponse((bufferedResponse) => {
              if (bufferedResponse.statusCode > 300) {
                return fail(503, bufferedResponse.body);
              }
              return cb(false, [
