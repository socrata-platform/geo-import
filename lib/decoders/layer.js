...
class Layer extends Duplex {
  ...
  belongsIn(soqlRow) {
    for(var i = 0; i < this.columns.length; i++) {
      ...
      let isEmpty = (soqlValue.value === null);
      if(!typeMatch && !isEmpty) return false;
    }
    return true;

    //converted to for loop becasue speed
    // return _.every(_.zip(this.columns, soqlRow), ([column, soqlValue]) => {
      //if the value is null, and the names match, then we don't know what the type
      //of the value is supposed to be - so we'll assume they are together.
      // return nameMatch && (typeMatch || isEmpty);
    // });
  }
  ...
  write(crs, soql, throwaway) {
    ...
    if(this.bufferedRows.length > 100) {
      this._spillToDisk();
    }
  }
