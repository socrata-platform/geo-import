...
class Layer {
  ...
  _expandBbox(geom) {
    ...
    geom.mapCoordinates(([x, y]) => {
      this._bbox.minx = Math.min(x, this._bbox.minx);
      this._bbox.miny = Math.min(y, this._bbox.miny);

      this._bbox.maxx = Math.max(x, this._bbox.maxx);
      this._bbox.maxy = Math.max(y, this._bbox.maxy);
    }.bind(this));
  }
