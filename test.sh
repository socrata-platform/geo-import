...
while inotifywait -r -e modify ./es6-lib ./es6-test; do
  ...
  then
    ...
    GEO_IMPORT_ENV=test ./node_modules/.bin/mocha es6-test/unit --require @babel/register
  else
    echo "Running tests that match $1"
    GEO_IMPORT_ENV=test ./node_modules/.bin/mocha es6-test/unit es6-test/smoke --grep "$1" --require @babel/register
  fi
  jshint es6-lib
done
